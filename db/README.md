...
## 결과 파일을 넣는 방법 (2026-09-10 반영)

받은 파일은 **반출 폴더에 덮지 않는다.** 프로젝트 루트의 `algo_in/` 에 두면
생성기가 같은 이름을 먼저 읽는다.

```
algo_in/05_속성판정_파생.csv     ← 받은 것을 여기 둔다
algo_in/06_적정재고_파생.csv
algo_in/요약값.json
본선_반출 최종본/데이터/...        ← 원본. 읽기 전용으로 남긴다
```

```bash
python validate_algorithm_csv.py algo_in        # 검문 · db/VALIDATE_REPORT.txt
PYTHONHASHSEED=0 PYTHONIOENCODING=utf-8 python -B build_master_db.py
```

빌드 리포트 `[10]` 이 **「받은 파일을 썼으므로 위 불일치는 알고리즘이 바꾼 값이다」**
라고 적고, 어느 칸이 몇 건 바뀌었는지 세어 준다. 반출본만 쓸 때의 불일치는
여전히 실패로 잡는다 (우리 재생성 버그를 놓치면 안 된다).

### 이번에 받은 결과에서 확인된 차이

| 표.칸 | 건수 | 뜻 |
|---|---|---|
| `attr.stockSrc` | 743 | 출처 라벨 (`Dummy TSV` → `DOG Type TSV + DB 자재정본`) |
| `attr.si` | 2 | Q2104247 · Q4079757 의 보험품 점수 42.9 → 14.3 |
| `stock.signal` `status` | 각 6 | `red/즉시발주` → `gray/재고충분` |
| `stock.expect` | 742 | **반출은 743행 전부 값 · 받은 것은 56행만** |
| `stock.sigEq` | 13 | 신호를 만든 설비명 |
| `stock.issues` | 2 | |

**신호 6건은 받은 쪽이 명세에 맞다.** 명세 `signalPln` 이
「현재고 >= 예상소요 → 재고충분」 을 먼저 적고 그 다음에 `D <= 0 → 즉시발주` 를 적는다.
그 6건은 예상 소요가 0 인데 반출본은 즉시발주로 뒀다. 살 것이 없는데 「지금 사라」 고
하면 담당자가 무엇을 살지 알 수 없다.

**`expect` 는 확인이 필요하다.** 정비계획에 걸린 56행에만 값이 있어서, 화면의
정비계획 소요 자재 표가 목표재고 기준(`need`)으로 부족분을 세도록 바꿨다.
예상 소요를 743행 전부 채울 수 있으면 표가 「이 정비 건 기준」 으로 더 정확해진다.
